...
describe("LcmContextEngine.bootstrap", () => {
  ...
  it("does not advance the bootstrap checkpoint when reconcile aborts at the import cap", async () => {
    ...
    expect(warnLog).toHaveBeenCalledWith(
      `[lcm] reconcileSessionTail: import cap chunking for conversation=${conversation!.conversationId} session=${sessionId} sessionKey=${sessionKey} — importing 50/60 anchored backlog messages this pass (existing: 2, cap: 50); remaining backlog continues next pass`,
    );
