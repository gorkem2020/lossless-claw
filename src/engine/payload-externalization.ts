...
type TextSummarizer = (prompt: string) => Promise<string | null>;

type PayloadExternalizerOptions = {
  config: LcmConfig;
  summaryStore: SummaryStore;
  resolveLargeFileTextSummarizer: () => Promise<TextSummarizer | undefined>;
};
...
/** Externalize oversized inline payloads into durable large-file records. */
export class PayloadExternalizer {
  private readonly config: LcmConfig;
  private readonly summaryStore: SummaryStore;
  private readonly resolveLargeFileTextSummarizer: () => Promise<TextSummarizer | undefined>;
  ...
  constructor(options: PayloadExternalizerOptions) {
    this.config = options.config;
    this.summaryStore = options.summaryStore;
    this.resolveLargeFileTextSummarizer = options.resolveLargeFileTextSummarizer;
  }

  private static detectBase64ImageType(
    base64Data: string,
  ): { extension: string; mimeType: string } | null {
    for (const sig of PayloadExternalizer.BASE64_IMAGE_MAGIC) {
  ...
  private static normalizeNativeImageBlock(value: unknown): {
    ...
    const detected = PayloadExternalizer.detectBase64ImageType(base64Data);
    const mimeType = detected?.mimeType ?? declaredMimeType;
    if (!mimeType?.toLowerCase().startsWith("image/")) {
      return null;
    }

    const extension = detected?.extension ?? PayloadExternalizer.extensionForImageMimeType(mimeType);
  ...
  private static inferNativeImageFileName(params: {
    ...
    for (let index = params.imageIndex - 1; index >= 0; index -= 1) {
      ...
      const fileName = mediaMatch?.[1]
        ? PayloadExternalizer.basenameForImageReference(mediaMatch[1])
        : null;
      if (fileName) {
        return fileName;
      }
  ...
  async interceptNativeUserImageBlocks(params: {
    conversationId: number;
    ...
    for (let index = 0; index < params.message.content.length; index += 1) {
      ...
      const image = PayloadExternalizer.normalizeNativeImageBlock(block);
      if (!image) {
      ...
      const externalized = await this.externalizeImage({
        ...
        fileName: PayloadExternalizer.inferNativeImageFileName({
  ...
  private async interceptPureBase64Image(params: {
    ...
    const detected = PayloadExternalizer.detectBase64ImageType(trimmed);
    if (!detected) {
      return null;
    }
  ...
  async interceptLargeToolResults(params: {
    ...
    for (const item of params.message.content) {
      ...
      if (
        ...
        PayloadExternalizer.isExternalizedImageReference(extractedText)
      ) {
        rewrittenContent.push(item);
        continue;
      }
  ...
  async interceptLargeRawPayload(params: {
    ...
    if (PayloadExternalizer.isExternalizedReferenceContent(params.stored.content)) {
      return null;
    }
